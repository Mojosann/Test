...
def make_square_layer(root, height):
	...
	# group cube_name
	#group_name = 
	#cmds.group([cube_name for cube_name in cube_name_list], n='Square_layer_%s' % root) #parent=tree_root
...
query_trunk_height = input('Enter trunk height: ')
trunk_height = make_trunk(query_trunk_height)


query_tree_layer = int(input('Enter tree layer num: '))
for query_tree_layer in range(query_tree_layer):
	tree_layer_list = [i for i in range(1, 7)][::-1]
	for layer in tree_layer_list:
		make_square_layer(layer, trunk_height)
		trunk_height += 1