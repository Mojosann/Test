...
class Tree_Window:
	...
	def __init__(self):
		...
		# create new window
		cmds.window('Create Tree', widthHeight=(400, 160))
		...
		self.tree_layer = cmds.intSliderGrp(field=True, label='Tree layer  ', minValue=1, maxValue=10, value=3)
		
		# set trunk_height slider 
		self.trunk_height = cmds.intSliderGrp(field=True, label='Trunk height  ', minValue=1, maxValue=10, value=7)
		
		# set create_tree button and connect create_tree()
		self.create_tree = cmds.button(label='Create Tree', command=self.create_tree)
	...
	def create_tree(self, *args):
		...
		# 0.5: YMAX of cube
		height = cmds.intSliderGrp(self.trunk_height, query=True, value=True)+0.5
			
		# get tree and trunk group name
		trunk = make_trunk(cmds.intSliderGrp(self.trunk_height, query=True, value=True))
		tree  = make_tree_layer(cmds.intSliderGrp(self.tree_layer, query=True, value=True), height)
